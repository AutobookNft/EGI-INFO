/**
 * Mattoncino: Problema 4 - Speculazione Vuota
 * Chiave JSON: florence.problems.items[3]
 * Rotta: /info/florence/problem-4
 */
...
const Problem4: React.FC = () => {
  ...
  const highlights = t('problems.items.3.highlights', { returnObjects: true }) as Array<{ icon: string; title: string; desc: string }>;
    ...
    <section style={{ minHeight: '100vh', padding: '80px 20px', background: '#0a0a0a' }}>
      <div style={{ maxWidth: '900px', margin: '0 auto' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '24px', marginBottom: '50px' }}>
          <div style={{ background: 'rgba(248, 113, 113, 0.08)', border: '2px solid rgba(248, 113, 113, 0.3)', borderRadius: '20px', padding: '32px', position: 'relative' }}>
            ...
            <div style={{ marginTop: '10px' }}>
              <p style={{ fontSize: '1.1rem', color: 'rgba(255, 255, 255, 0.6)', marginBottom: '16px' }}>{t('problems.items.3.old')}</p>
              <div style={{ fontSize: '3rem', marginBottom: '8px' }}>{t('problems.items.3.oldValue')}</div>
              <p style={{ color: 'rgba(255, 255, 255, 0.5)', fontSize: '0.9rem' }}>{t('problems.items.3.oldCaption')}</p>
            </div>
          </div>
          <div style={{ background: 'rgba(74, 222, 128, 0.08)', border: '2px solid rgba(74, 222, 128, 0.3)', borderRadius: '20px', padding: '32px', position: 'relative' }}>
            ...
            <div style={{ marginTop: '10px' }}>
              <p style={{ fontSize: '1.1rem', color: 'rgba(255, 255, 255, 0.85)', marginBottom: '16px' }}>{t('problems.items.3.new')}</p>
              <div style={{ fontSize: '3rem', marginBottom: '8px' }}>{t('problems.items.3.newValue')}</div>
              <p style={{ color: 'rgba(255, 255, 255, 0.7)', fontSize: '0.9rem' }}>{t('problems.items.3.newCaption')}</p>
            </div>
          </div>
        </div>
        ...
        <div style={{ background: 'rgba(30, 30, 40, 0.7)', border: '1px solid rgba(255, 255, 255, 0.1)', borderRadius: '24px', padding: '40px', marginBottom: '30px' }}>
          ...
          <p style={{ fontSize: '1.1rem', color: 'rgba(255, 255, 255, 0.85)', lineHeight: 1.8 }}>{t('problems.items.3.detailProblem')}</p>
        </div>
        ...
        <div style={{ background: 'rgba(74, 222, 128, 0.06)', border: '1px solid rgba(74, 222, 128, 0.2)', borderRadius: '24px', padding: '40px', marginBottom: '40px' }}>
          ...
          <p style={{ fontSize: '1.1rem', color: 'rgba(255, 255, 255, 0.85)', lineHeight: 1.8 }}>
            {t('problems.items.3.detailSolution')}
          </p>
        </div>
